...
function Services:Get(...)
    ...
    for i, serviceName in ipairs(services) do
        if not self._cache[serviceName] then
            self._cache[serviceName] = game:GetService(serviceName)
        end
        results[i] = self._cache[serviceName]
    end
    
    return unpack(results)
end

-- Simple KeyBindVisualizer implementation
...
local function toCamelCase(str)
    return str:gsub("_%l", string.upper):gsub("^%l", string.upper)
end
