...
do
	...
	do
		...
		function Library:Window(Options)
			...
			local LeftHolder = Instance.new('Frame', Sections)
			local RightHolder = Instance.new('Frame', Sections)
			...
			Sections.Position = UDim2.new(0, 10, 0, 10)
			Sections.Size = UDim2.new(1, -20, 1, -20)
			Sections.BackgroundTransparency = 1
			
			LeftHolder.Name = "LeftHolder"
			LeftHolder.Position = UDim2.new(0, 0, 0, 0)
			LeftHolder.Size = UDim2.new(0.5, -5, 1, 0)
			LeftHolder.BackgroundTransparency = 1
			
			RightHolder.Name = "RightHolder"
			RightHolder.Position = UDim2.new(0.5, 5, 0, 0)
			RightHolder.Size = UDim2.new(0.5, -5, 1, 0)
			...
			ResizeHandle.MouseButton1Down:Connect(function()
				Base.Resizing = true
				local startPos = game:GetService("UserInputService"):GetMouseLocation()
				local startSize = Main.Size
				
				local connection
				connection = game:GetService("RunService").RenderStepped:Connect(function()
					if not Base.Resizing then
						connection:Disconnect()
						return
					end
			...
			function Base:Section(Options)
				local SectionInstance = {
					Window = self,
					Elements = {}
				}

				local Holder = Instance.new('Frame', Options.Side == "Left" and LeftHolder or RightHolder)
				Holder.Name = Options.Name or "Section"
				...
				Title.Text = Options.Name or "Section"
				...
				SectionInstance.Elements = Container
				return setmetatable(SectionInstance, Library.Sections)
			end
